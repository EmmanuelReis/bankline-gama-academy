...
public class UsuarioServiceImpl implements UsuarioService {
	...
	private void validar(UsuarioDTO usuario) throws Exception {
		...
		if (usuario == null) {
			throw new BanklineApiException(ErrorCode.E0001, "usuario", null, null);
		}

		if (StringUtils.isBlank(usuario.getLogin())) {
			throw new BanklineApiException(ErrorCode.E0002, "usuario", "login", null);
		}

		v = map.get("login");
		if (usuario.getLogin().length() < v.getMin() || usuario.getLogin().length() > v.getMax()) {
			throw new BanklineApiException(ErrorCode.E0008, "usuario", "login", "" + usuario.getLogin().length(),
		...
		if (StringUtils.isBlank(usuario.getCpf())) {
			throw new BanklineApiException(ErrorCode.E0002, "usuario", "cpf", null);
		}

		v = map.get("cpf");
		if (usuario.getCpf().length() < v.getMin() || usuario.getCpf().length() > v.getMax()) {
			throw new BanklineApiException(ErrorCode.E0008, "usuario", "cpf", "" + usuario.getCpf().length(),
					Operadores.MAIOR_IGUAL.getOperador(), v.getMin().toString(), Operadores.MENOR_IGUAL.getOperador(),
					v.getMax().toString());
		}

		if (StringUtils.isBlank(usuario.getSenha())) {
			throw new BanklineApiException(ErrorCode.E0002, "usuario", "senha", null);
		}

		v = map.get("senha");
		if (usuario.getSenha().length() < v.getMin() || usuario.getSenha().length() > v.getMax()) {
			throw new BanklineApiException(ErrorCode.E0008, "usuario", "senha", "" + usuario.getSenha().length(),
					Operadores.MAIOR_IGUAL.getOperador(), v.getMin().toString(), Operadores.MENOR_IGUAL.getOperador(),
					v.getMax().toString());
		}

		if (StringUtils.isBlank(usuario.getNome())) {
			throw new BanklineApiException(ErrorCode.E0002, "usuario", "nome", null);
		}

		v = map.get("nome");
		if (usuario.getNome().length() < v.getMin() || usuario.getNome().length() > v.getMax()) {
			throw new BanklineApiException(ErrorCode.E0008, "usuario", "nome", "" + usuario.getNome().length(),
					Operadores.MAIOR_IGUAL.getOperador(), v.getMin().toString(), Operadores.MENOR_IGUAL.getOperador(),
					v.getMax().toString());
		}
